...
public  class AsteroidEnemy extends Enemy {
    ...
    public AsteroidEnemy (PlayScreen screen, float x, float y) {
        super(screen, x, y);
        this.screen = screen;

        frames = new Array<TextureRegion>();
        for (int i = 0; i < 16; i++)
            frames.add(new TextureRegion(screen.getAtlas4().findRegion("rock0"), i * 66, 0, 64, 64));
        ...
        /*for (int e = 0; e < 8; e++)
            frames.add(new TextureRegion(screen.getAtlas9().findRegion("6"), e * 102, 0, 86, 86));
        destroydEnemy = new Animation(0.1f, frames);*/

       /* for (int e = 0; e < 46; e++)
            frames.add(new TextureRegion(screen.getAtlas9().findRegion("13"), e * 82, 0, 84, 84));
        destroydEnemy = new Animation(0.050f, frames);
*/


        shipStand = new TextureRegion(screen.getAtlas4().findRegion("rock0") ,64,0,64,64 );
